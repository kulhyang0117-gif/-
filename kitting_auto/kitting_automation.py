...
def _click_excel_btn(win, row_y=None):
    ...
    #    조회/Excel/종료 C1Command 버튼은 텍스트 탐지 불가
    #    → pnl_top_button 너비 50% 지점 = 2번째(Excel) 버튼 중앙
    try:
        ...
        if _src_rect:
            # btn_src_hide 기준 (키팅 폼 내부 앵커 — 우선 사용)
            ...
            # pnl_top_button fallback (메인 앱 리본일 수 있음 — 주의)
...
def _download_by_keyboard(win):
    ...
    # ROW_X: 창 너비의 35% (듀얼 모니터 스팬 시 중앙이 모니터 경계에 걸리는 문제 방지)
    try:
